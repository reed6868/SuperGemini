...
# SuperGemini Installer Script
# Installs SuperGemini configuration framework to enhance Gemini CLI
# Version: 1.0.0
# License: MIT
# Repository: https://github.com/reed6868/SuperGemini
...
readonly CONFIG_FILE=".SuperGemini.conf"
...
readonly -a CUSTOMIZABLE_CONFIGS=("GEMINI.md" "RULES.md" "PERSONAS.md" "MCP.md")
...
# 依赖检查
check_dependencies() {
    local node_version npm_version gemini_version
    if ! check_command node; then
        echo -e "${RED}Error: Node.js is required but not found.${NC}"
        exit 1
    fi
    node_version=$(node -v | grep -Eo '[0-9]+\.[0-9]+\.[0-9]+')
    if [[ -z "$node_version" ]] || [[ "${node_version%%.*}" -lt 20 ]]; then
        echo -e "${RED}Error: Node.js v20+ is required. Detected: $node_version${NC}"
        exit 1
    fi
    ...
    npm_version=$(npm -v | grep -Eo '^[0-9]+')
    if [[ -z "$npm_version" ]] || [[ "$npm_version" -lt 8 ]]; then
        echo -e "${RED}Error: npm v8+ is required. Detected: $npm_version${NC}"
        exit 1
    fi
    if ! check_command gemini; then
        echo -e "${RED}Error: Gemini CLI is required but not found. Please run: npm install -g @google/gemini-cli${NC}"
        exit 1
    fi
    gemini_version=$(gemini --version | grep -Eo '[0-9]+\.[0-9]+\.[0-9]+')
...
EXCEPTION_PATTERNS=(
    ...
    "backup.*"
    ".git*"
    "GEMINI.md"  # User might customize main config
...
# Returns: None
show_usage() {
    echo "SuperGemini Installer v$SCRIPT_VERSION"
    echo ""
    echo "Usage: $0 [OPTIONS]"
    echo ""
    echo "Options:"
    echo "  --dir <directory>    Install to custom directory (default: $HOME/.gemini)"
    echo "  --force              Skip confirmation prompts (for automation)"
    echo "  --update             Update existing installation (preserves customizations)"
    echo "  --uninstall          Remove SuperGemini from specified directory"
    ...
    echo "  --check-update       Check for SuperGemini updates"
    echo "  --version            Show installer version"
    echo "  -h, --help          Show this help message"
    echo ""
    echo "Examples:"
    echo "  $0                          # Install to default location"
    echo "  $0 --dir /opt/gemini        # Install to /opt/gemini"
    echo "  $0 --dir ./local-gemini     # Install to ./local-gemini"
    echo "  $0 --force                  # Install without prompts"
    echo "  $0 --update                 # Update existing installation"
    echo "  $0 --uninstall              # Remove SuperGemini"
...
get_source_files() {
    (  # Run in subshell to isolate directory changes
        ...
        # Validate that .gemini directory exists
        if [[ ! -d ".gemini" ]]; then
            log_error "get_source_files: .gemini directory not found in source root"
            return 1
        fi
        
        # Find all files in .gemini directory and map them to root with error handling
        file_list=""
        if ! file_list=$(find .gemini -type f \
            -not -path "*/.git*" \
            -not -path "*/backup.*" \
            -not -path "*/log/*" \
            -not -path "*/logs/*" \
            ...
            -not -name "GEMINI.md" \
            2>/dev/null | sed 's|^\.gemini/||' | sort); then
            log_error "get_source_files: Failed to enumerate files in .gemini directory"
            return 1
        fi
        ...
        # Also include GEMINI.md from root if it exists
        if [[ -f "GEMINI.md" ]]; then
            echo "GEMINI.md"
        fi
...
# Description: Check for SuperGemini updates from GitHub
# Parameters: None
# Returns: 0 if update available, 1 if up to date, 2 on error
check_for_updates() {
    local repo_url="https://api.github.com/repos/reed6868/SuperGemini/releases/latest"
    ...
    log "Checking for SuperGemini updates..."
    ...
    if compare_versions "$SCRIPT_VERSION" "$latest_version"; then
        ...
        echo "Download: https://github.com/reed6868/SuperGemini/releases/latest"
        return 0
    else
...
load_default_config() {
    # System-wide config
    if [[ -f "/etc/SuperGemini.conf" ]]; then
        load_config "/etc/SuperGemini.conf"
    fi
    
    # User config
    if [[ -f "$HOME/.SuperGemini.conf" ]]; then
        load_config "$HOME/.SuperGemini.conf"
    fi
    
    # Local config
    if [[ -f ".SuperGemini.conf" ]]; then
        load_config ".SuperGemini.conf"
    fi
}
...
if [[ "$UNINSTALL_MODE" = true ]]; then
    echo -e "${GREEN}SuperGemini Uninstaller${NC}"
    echo "========================"
    echo -e "Target directory: ${YELLOW}$INSTALL_DIR${NC}"
    echo ""
    
    if [[ ! -d "$INSTALL_DIR" ]]; then
        echo -e "${RED}Error: SuperGemini not found at $INSTALL_DIR${NC}"
        exit 1
    fi
    
    if [[ "$FORCE_INSTALL" != true ]]; then
        echo -e "${YELLOW}This will remove SuperGemini from $INSTALL_DIR${NC}"
    ...
    echo "Removing SuperGemini files while preserving user data..."
    
    # Remove SuperGemini files but preserve user data
    removed_count=0
    preserved_count=0
    
    # First, remove all SuperGemini files (those that would be installed)
    if [[ -d "$INSTALL_DIR" ]]; then
        ...
        while IFS= read -r installed_file; do
            ...
            if is_preserve_file "$installed_file"; then
                ...
                # Check if this file exists in source (is a SuperGemini file)
                ...
                if [[ -f "$current_dir/.gemini/$installed_file" ]] || 
                   [[ "$installed_file" == "GEMINI.md" && -f "$current_dir/GEMINI.md" ]] || 
        ...
        if [[ -z "$(ls -A "$INSTALL_DIR" 2>/dev/null)" ]]; then
            ...
            echo -e "${GREEN}✓ SuperGemini uninstalled completely!${NC}"
        else
            echo ""
            echo -e "${GREEN}✓ SuperGemini uninstalled successfully!${NC}"
...
if [[ "$VERIFY_MODE" = true ]]; then
    echo -e "${GREEN}SuperGemini Verification${NC}"
    echo "========================="
    echo -e "Target directory: ${YELLOW}$INSTALL_DIR${NC}"
    echo ""
    ...
    # Check if we're in SuperGemini directory
    if [ ! -f "GEMINI.md" ] || [ ! -d ".gemini" ] || [ ! -d ".gemini/commands" ]; then
        echo -e "${RED}Error: This script must be run from the SuperGemini directory${NC}"
        echo ""
        echo "Expected files not found. Please ensure you are in the root SuperGemini directory."
        echo "Missing: $([ ! -f "GEMINI.md" ] && echo "GEMINI.md ")$([ ! -d ".gemini" ] && echo ".gemini/ ")$([ ! -d ".gemini/commands" ] && echo ".gemini/commands/")"
        echo ""
        echo "Solution: cd to the SuperGemini directory and run: ./install.sh --verify-checksums"
        exit 1
    fi
...
echo -e "${GREEN}SuperGemini Installer${NC}"
echo "======================"
...
if [[ "$FORCE_INSTALL" != true ]]; then
    if [[ "$UPDATE_MODE" = true ]]; then
        echo -e "${YELLOW}This will update SuperGemini in $INSTALL_DIR${NC}"
    else
        echo -e "${YELLOW}This will install SuperGemini in $INSTALL_DIR${NC}"
    fi
    echo -n "Are you sure you want to continue? (y/n): "
...
if [ ! -f "GEMINI.md" ] || [ ! -d ".gemini" ] || [ ! -d ".gemini/commands" ]; then
    ...
    echo "Missing: $([ ! -f "GEMINI.md" ] && echo "GEMINI.md ")$([ ! -d ".gemini" ] && echo ".gemini/ ")$([ ! -d ".gemini/commands" ] && echo ".gemini/commands/")"
    echo ""
    echo "Solution: cd to the SuperGemini directory and run: ./install.sh"
    exit 1
fi

# Get version information
SuperGemini_VERSION="unknown"
if [[ -f "VERSION" ]] && [[ -r "VERSION" ]]; then
    SuperGemini_VERSION=$(< VERSION) || SuperGemini_VERSION="unknown"
fi
log_verbose "SuperGemini version: $SuperGemini_VERSION"
...
if [[ -f "$INSTALL_DIR/VERSION" ]] && [[ -r "$INSTALL_DIR/VERSION" ]]; then
    ...
    if [[ "$UPDATE_MODE" = true ]]; then
        ...
        echo "New version: $SuperGemini_VERSION"
        echo ""
    fi
fi
...
if [ -d "$INSTALL_DIR" ] && [ "$(ls -A "$INSTALL_DIR" 2>/dev/null)" ]; then
    ...
    if [ "$backup_choice" = "y" ]; then
        ...
        BACKUP_DIR="$(dirname "$INSTALL_DIR")/SuperGemini-backup.${backup_timestamp}.${backup_random}"
        ...
        find . -mindepth 1 -maxdepth 1 \( -name "SuperGemini-backup.*" -prune \) -o -print0 | \
...
if [[ "$DRY_RUN" != true ]]; then
    ...
    # Find all directories in .gemini and create them in destination (without .gemini prefix)
    find .gemini -type d \
        -not -path "*/.git*" \
        -not -path "*/backup.*" \
        -not -path "*/log" \
...
if command -v sha256sum &> /dev/null && [[ "$DRY_RUN" != true ]]; then
    ...
    done < <(find . -type f -not -path "./SuperGemini-backup.*" -not -name ".checksums" | sort)
...
gemini_shared=$(find "$INSTALL_DIR/shared" -name "*.yml" -type f 2>/dev/null | wc -l)
...
echo -e "  Gemini shared files: ${GREEN}$gemini_shared${NC}"

# Verify critical files exist
critical_files_ok=true
for critical_file in "GEMINI.md" "commands" "shared"; do
...
if [ "$actual_files" -ge "$expected_files" ] && [ "$critical_files_ok" = true ] && [ $VERIFICATION_FAILURES -eq 0 ]; then
    ...
    echo ""
    if [[ "$UPDATE_MODE" = true ]]; then
        echo -e "${GREEN}✓ SuperGemini updated successfully!${NC}"
        echo ""
        # Check for .new files
        ...
        echo -e "${GREEN}✓ SuperGemini installed successfully!${NC}"
        echo ""
        echo "Next steps:"
        echo "1. Open any project with Gemini Code"
